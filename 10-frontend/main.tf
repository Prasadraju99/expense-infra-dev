...
resource "aws_autoscaling_policy" "frontend" {
  name                   = local.resource_name  # terraform apply -target=aws_autoscaling_group.frontend
...
resource "aws_lb_listener_rule" "frontend" {
  ...
  condition {
    host_header {
      values = ["expense-${var.environment}.${var.zone_name}"]  # expense-dev.prasadking.xyz
    }
  }
}